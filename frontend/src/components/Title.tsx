...
export default function Title({ subTitle, mainTitle }: { subTitle: string, mainTitle: string }) {
    return (
        <div className="relative w-full h-36 flex justify-center items-center flex-col">
            ...
            <h3 className={`text-white ${caveat.className} text-2xl tracking-widest`}>{subTitle}</h3>
            <h2 className={`text-mainLight ${sumana.className} text-2xl font-bold uppercase tracking-[0.3em]`}>{mainTitle}</h2>
        </div>
    )
}